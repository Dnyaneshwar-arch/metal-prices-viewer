...
def render_metal_prices_page():
    ...
    scrap_chart_key = (
        f"scrap-{slug}-{start.isoformat()}-"
        f"{end.isoformat()}-{ver}-{global_ver}"
    )
...
def render_billet_prices_page():
    ...
    chart2 = _tidy((bars2 + line2_forecast + line2_actual).properties(height=430))
...
def render_grade_prices_page():
    ...
    raw = xls.parse(sheet_name=sel_sheet)
    ...
    if month_col is None or price_col is None:
        st.error("Could not detect a Month and Price column.")
        st.stop()
    ...
    months = df["MonthText"].tolist()

    bars = (
        alt.Chart(df)
        .mark_bar(size=_bar_size(len(df)))
        .encode(
            x="MonthText:N",
            y="Price:Q",
            tooltip=[
                alt.Tooltip("MonthText:N", title="Month"),
                alt.Tooltip("Price:Q", title="Price"),
            ],
        )
    )

    st.altair_chart(_tidy(bars.properties(height=430)), use_container_width=True)
